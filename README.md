...
[![coverage](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fnousergon%2Fsymposion%2Fbadges%2Fcoverage.json)](https://github.com/nousergon/symposion/actions/workflows/coverage-badge.yml)
...
[![License](https://img.shields.io/github/license/nousergon/symposion)](LICENSE)
[![Node](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fraw.githubusercontent.com%2Fnousergon%2Fsymposion%2Fmain%2Fpackage.json&query=%24.engines.node&label=node&color=blue)](https://nodejs.org/)
...
- **API-backed personas** (OpenRouter, DeepSeek, xAI, or any other provider [OpenCode](https://opencode.ai) supports) run through OpenCode's own server, fronted via its SDK — this backend has a genuine live permission-pause/resume flow (`permission.asked`/`question.asked`), surfaced in the UI as an actionable card, unlike the Claude Code path (which has no mid-turn pause in headless mode — a blocked action shows up after the fact as `permission_denials` on the completed turn). API-backed personas address a **capability class** (`low`/`med`/`high`/`ultra`), never a concrete model, and reach it through the fleet's LiteLLM router on `127.0.0.1:8980` — which owns model selection, fallback chains, and routing through the shared content-scanning DLP egress proxy. Symposion holds no model list, provider map, or endpoint of its own; the registry decides what each class means. See [`nous-ergon-ops/policies/model-router-policy.md`](https://github.com/nousergon/nous-ergon-ops/blob/main/policies/model-router-policy.md).
